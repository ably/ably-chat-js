...
export interface Presence {
   ...
   *   console.log(`${event.member.clientId} entered the room`);
   *   console.log('User data:', event.member.data);
   * });
   *
   * // Subscribe to multiple event types
   * const subscription = room.presence.subscribe(['enter', 'leave', 'update'],
   *   (event: PresenceEvent) => {
   *     switch (event.type) {

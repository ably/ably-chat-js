...
import { mockReturningPromiseNull } from '../helper/promise.js';
...
describe('RoomLifecycleManager', () => {
  beforeEach<TestContext>((context) => {
    ...
    context.mockChannel = {
      attach: vi.fn(mockReturningPromiseNull()),
      detach: vi.fn(mockReturningPromiseNull()),
      state: 'initialized',
  ...
  describe('detach', () => {
    ...
    it<TestContext>('should successfully detach and update status', async ({
      ...
      // Arrange
      roomStatus.setStatus({ status: RoomStatus.Attached });
      mockChannel.detach = vi.fn(mockReturningPromiseNull());
  ...
  describe('channel state monitoring', () => {
    ...
    it<TestContext>('should ignore channel state changes when attach operation is in progress', async ({
      ...
      mockChannel.attach = vi.fn().mockImplementation(async () =>
        vi.waitFor(() => {
          attachCalled = true;
    ...
    it<TestContext>('should ignore channel state changes when detach operation is in progress', async ({
      ...
      mockChannel.detach = vi.fn().mockImplementation(async () =>
        vi.waitFor(() => {
          detachCalled = true;

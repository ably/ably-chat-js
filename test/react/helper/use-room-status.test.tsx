...
describe('useRoomStatus', () => {
    ...
    it('sets instantaneous room state values', () => {
      ...
      // We should have initialized and no error
        expect(result.current.status).toBe(RoomLifecycle.Initialized);
    ...
    it('sets room status values after useEffect', async () => {
        ...
        (mockRoom.status as InternalRoomStatus).setStatus({status: RoomLifecycle.Failed, error});
        ...
        await waitForEventualHookValue(result, RoomLifecycle.Failed, (value) => value.status);
    ...
    it('subscribes to changing room status', async () => {
        ...
        (mockRoom.status as InternalRoomStatus).setStatus({status: RoomLifecycle.Detached, error: newError});

        // Now wait until the hook has updated the status and error
        await waitForEventualHookValue(result, RoomLifecycle.Detached, (value) => value.status);
    ...
    it('subscribes user-provided listeners to changing room status', async () => {
        ...
        // Render the hook
        const {unmount} = renderHook(() => useRoomStatus({
        ...
        expect((mockRoom.status as unknown as {listeners(): unknown[]}).listeners()).toHaveLength(2);

        // Check the event
        expect(receivedEvents[0]?.current).toBe(RoomLifecycle.Failed);
        expect(receivedEvents[0]?.previous).toBe(RoomLifecycle.Initializing);
        ...
        await vi.waitFor(() => { expect(receivedEvents).toHaveLength(2); });

        // Check the event
        expect(receivedEvents[1]?.current).toBe(RoomLifecycle.Detached);
        expect(receivedEvents[1]?.previous).toBe(RoomLifecycle.Failed);
        expect(receivedEvents[1]?.error).toBe(newError);

        // After unmount we should have no listeners
        unmount();
        expect((mockRoom.status as unknown as {listeners(): unknown[]}).listeners()).toBeNull();
    });
});

...
describe('ChatRoomProvider', () => {
  ...
  it('should create a provider without error', async () => {
    ...
    await expect(() => chatClient.rooms.get(roomId, DefaultRoomOptions)).rejects.toBeErrorInfoWithCode(40000);

    // Now try it with the right options, should be fine
    await chatClient.rooms.get(roomId, { reactions: DefaultRoomOptions.reactions });
  });
  ...
  it('should correctly release rooms', async () => {
    ...
    // Now try it with the right options, should be fine
    expect(() => chatClient.rooms.get(roomId, { reactions: DefaultRoomOptions.reactions }));
    ...
    expect(() => chatClient.rooms.get(roomId, DefaultRoomOptions)).toBeTruthy();
  });
